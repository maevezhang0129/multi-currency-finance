...
崩溃兜底页（崩了也能导出原始数据）、账本版本迁移脚手架、集成测试、
PWA（可装到主屏幕）+ service worker（断网可用）+ 持久化存储申请。
...
**第三阶段：先用，再改（2026-08-12 起）**

功能开发到此暂停。下一步不是写代码，是**连续真实记账两周**，把每次觉得
「烦」的瞬间记下来，两周后拿那张清单决定做什么。理由是第 8 节开头那句话：
用不顺的地方比没做完的功能更值得优先处理，而「用不顺在哪」只有用了才知道。

**当前状态**：231 个测试、CI 绿、线上运行中（https://multi-currency-finance.vercel.app）。
README 已按作品集形态重写（英文为主 `README.md`，中文 `README.zh-CN.md`）。

已知的三处债，不急但别忘（2026-08-12 评估）：

1. **UI 层测试薄。** 组件共约 2300 行占源码 39%，只有 9 条集成测试覆盖。
   其中 `csv-import.tsx` 409 行零直接测试，而 CSV 导入恰是最容易出错的一段。
   下次出 bug 大概率在这一带。
2. **`ledger-app.tsx` 718 行**，全项目最大，且是唯一的状态所有者。现在还读得懂，
   再加两个功能就该拆。
3. **service worker 的资源缓存只涨不减。** `/_next/static/` 走缓存优先，每次发版
   产生新哈希文件名，旧的要等 `VERSION` 从 v1 改成 v2 才清。发版时顺手 bump，
   或给它加条数上限。
