...
5. **需求与界面设计**。先想清楚记一笔账的完整流程、要看到哪些分析，再画界面。
   不要跳过这一步直接写组件——返工的代价比想清楚的代价大得多。
6. **录入与存储**：localStorage 存储、手动录入一笔账、编辑与删除。
   录入速度是第一优先级，这是每天都要做的动作。
7. **折算与分析**：用交易发生月的汇率折算并冻结（见第 5 节），
   本币视角 / 基准币视角 / 汇率影响三行。
8. **导入导出**：CSV 导入，以及能导出全部数据（用户必须能带走自己的数据）。
